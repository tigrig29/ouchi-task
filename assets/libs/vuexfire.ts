...
const VfTaskList = {
  ...
  get convertVuexToFire() {
    return (vuexTaskList: VuexTaskList) => {
      ...
      const fireTaskList: FireTaskList = {
        ...
        lastResetAt: date.reversePickUpDate(lastResetAt || '2020-01-01') as any
      }
      return fireTaskList
    }
  },
  ...
  async addBoth(taskListNotHasId: VuexTaskList) {
    ...
    // Firestore へ Add
    const taskListId = await FmTaskList.add(userId, fireTaskList)
  ...
  async updateBoth(taskList: VuexTaskList) {
    ...
    // Firestore の Update
    await FmTaskList.update(userId, taskList.id, fireTaskList)
  },
  /**
   * Vuex, Firestore 両方の TaskList を更新する
   * @param taskList 新しい（更新後の） VuexTaskList
  ...
  async deleteBoth(taskList: VuexTaskList) {
    ...
    await FmTaskList.delete(userId, taskList.id)
...
const VfTask = {
  ...
  async addBoth(taskNotHasId: VuexTask) {
    ...
    // Firestore へ Add
    const taskId = await FmTask.add(userId, taskNotHasId.parentId, fireTask)
  ...
  async updateBoth(task: VuexTask) {
    ...
    // Firestore の Update
    await FmTask.update(userId, task.parentId, task.id, fireTask)
  },
  /**
   * Vuex, Firestore 両方の Task を更新する
   * @param task 新しい（更新後の） VuexTask
  ...
  async deleteBoth(task: VuexTask) {
    ...
    await FmTask.delete(userId, task.parentId, task.id)
    taskStore.delete({ taskId: task.id })
  }
}

export { VfTaskList, VfTask }

export default { taskList: VfTaskList, task: VfTask }
