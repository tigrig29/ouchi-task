...
const FmTaskList = {
  ...
  get docRef() {
    return async (userId: string, taskListId: string) => {
      ...
      if (!exists) docRef = this.collectionRef(userId, 'cards').doc(taskListId)
  ...
  async delete(userId: string, taskListId: string) {
    ...
    for (const task of taskStore.listFilteredByParentId(taskListId)) {
      promises.push(FmTask.delete(userId, taskListId, task.id))
    }
...
const FmTask = {
  ...
  get collectionRef() {
    return async (userId: string, taskListId: string) => {
      const docRef = await FmTaskList.docRef(userId, taskListId)
...
export { FmTaskList, FmTask }

export default { taskList: FmTaskList, task: FmTask }
